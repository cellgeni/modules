...
from pyro.infer import config_enumerate, infer_discrete, SVI, TraceEnum_ELBO
...
def barcodes_01_from_channels(barcodes_1234, C, R):
    ...
    for b in range(K):
        barcodes_01[b, :, :] = \
            1 * np.transpose(barcodes_1234[b, :].reshape(R, 1) == np.arange(1, C + 1))
    return barcodes_01
...
def kronecker_product(tr, tc):
    ...
    expanded_tr = \
        (tr.unsqueeze(2).unsqueeze(3).repeat(1, tc_height, tc_width, 1).view(out_height, out_width))
...
def map_states(data, N, D, C, R, K, codes, batch_size=None, temperature=0):
    inferred_model = \
        infer_discrete(poutine.replay(model_constrained_tensor,
                                      trace=poutine.trace(auto_guide).get_trace(data, N, D, C, R, K,
                                                                                codes)),
...
def model_constrained_tensor(data, N, D, C, R, K, codes, batch_size=None):
    ...
    codes_tr_v = pyro.param('codes_tr_v', 3 * torch.ones(1, D),
...
auto_guide = AutoDelta(poutine.block(model_constrained_tensor,
...
def decoding_function(spots, barcodes_01,
                      ...
                      modify_bkg_prior=True,  # should be False when there is a lot of background
                                              ...
                                              # boundary tiles)
                      estimate_bkg=True, estimate_additional_barcodes=None,  # controls adding
                                                                             # additional barcodes
                                                                             # during parameter
                                                                             # estimation
    ...
    svi = SVI(model_constrained_tensor, auto_guide, optim, loss=TraceEnum_ELBO(max_plate_nesting=1))
    pyro.set_rng_seed(set_seed)
    losses = train(svi, num_iter, data_norm[ind_keep, :], len(ind_keep), D, C, R, codes.shape[0],
                   codes, print_training_progress, min(len(ind_keep), batch_size))
    ...
    if add_remaining_barcodes_prior > 0:
        ...
        w_star_all = \
            torch.cat((alpha * w_star_mod,
                       torch.tensor((1 - alpha) / codes_inf.shape[0]).repeat(codes_inf.shape[0])))
        class_probs_star = \
            e_step(data_norm, w_star_all,
                   torch.matmul(torch.cat((codes, codes_inf)) * codes_tr_v_star
                                + codes_tr_consts_v_star.repeat(w_star_all.shape[0], 1),
                                mat_sqrt(sigma_star, D)),
                   sigma_star, N, w_star_all.shape[0], print_training_progress)
    else:
    ...
    if nan_spot_ind.shape[0] > 0:
        ...
        class_probs_star_s = torch.cat((class_probs_star_s,
                                       torch.zeros((class_probs_star_s.shape[0], 1))), dim=1)
    ...
    return {'class_probs': class_probs, 'class_ind': class_ind, 'params': torch_params,
