...
workflow BASIC_CORRECTION_ZARR {
    ...
    datas = Generate_ome_zarr_stub.out.fovs
        .splitJson()
        .multiMap {
    ...
    basic_models = BIOINFOTONGLI_BASICFITTING.out.basic_models
        .groupTuple(by: [0, 1])
        .map { it ->
            [it[0], it[1], it[2].sort()]
        }
    ...
    BIOINFOTONGLI_BASICTRANSFORM(
        datas.for_transforming.unique().combine(basic_models, by: [0, 1]).combine(Generate_ome_zarr_stub.out.ome_zarr_stub, by: 0)
    )
